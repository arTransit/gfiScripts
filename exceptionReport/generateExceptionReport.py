...
class GFIquery:
    ...
    def generateSQL(self,location,year,month):
        ...
        return (
            "select bus,trdate,trtime,route,drv,wm_concat(issue) as issue "
            "from ( "
                "select bus,TO_CHAR(ts,'YYYY-MM-DD HH24') groupdate,TO_CHAR(min(ts),'YYYY-MM-DD') trdate,TO_CHAR(min(ts),'HH24:MI') trtime,route,drv,'route' issue "
                "from tr "
                "where "
                    "loc_n in ( %s ) and "
                    "type in (115,118,119,129,143) and "
                    "ts between to_date('%s-%s-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and last_day(to_date('%s-%s-01 23:59:59', 'YYYY-MM-DD HH24:MI:SS')) and "
                    "route not in (select route from rtelst where loc_n in (%s) ) "
                "group by bus,TO_CHAR(ts,'YYYY-MM-DD HH24'),route,drv "
                "union "
                "select bus,TO_CHAR(ts,'YYYY-MM-DD HH24') groupdate,TO_CHAR(min(ts),'YYYY-MM-DD') trdate,TO_CHAR(min(ts),'HH24:MI') trtime,route,drv,'driver' issue "
                "from tr "
                "where "
                    "loc_n in ( %s ) and "
                    "type in (115,118,119,129,143) and "
                    "ts between to_date('%s-%s-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and last_day(to_date('%s-%s-01 23:59:59', 'YYYY-MM-DD HH24:MI:SS')) and "
                    "drv not in (select drv from drvlst where loc_n in (%s) ) "
                "group by bus,TO_CHAR(ts,'YYYY-MM-DD HH24'),route,drv "
            ") "
            "group by bus,trdate,trtime,route,drv "
            "order by bus,trdate,trtime,route,drv "
